...
extension TodayViewModel {
    ...
    func logWakeNow(reason: WakeReason,
        ...
        // Log to controller with wake context
        controller.logWake(
            sleepSessionKey: nightKey,
    ...
    private func ensureNightKeyMintedIfNeeded() {
        if nightKey == nil {
    ...
    private func endDose2LiveActivityIfRunning() {
        controller.endLiveActivity()
    }
    
    private func cancelDose2Notifications() {
        controller.cancelDose2Notifications()
    }
    
    private func haptics(_ style: UINotificationFeedbackGenerator.FeedbackType) {
        #if !os(macOS)
